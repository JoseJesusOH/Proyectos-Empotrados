...
const char* ssid = "UNO";
const char* password = "OnePiece123";
  ...
  while (true) {
    ...
    if (boton_finalizar.fell()) {
      ...
      if (units == 0.0 || units <= 0.1) {
        ...
        delay(2000);
        pantalla();
        vaciado = "Vaciado" comandoAnterior = "";
        break;
      } else {
...
void procesaComando(uint8_t* data, size_t len) {
  ...
  if (comando == "abrir" || comando == "cerrar" || comando == "vaciar" || comando == "fin" || comando == "state") {
    ...
    } else if (comandoAnterior == "vaciar" && comando != "fin") {
    ...
    if (comandoAnterior == "abrir" && comando == "cerrar" && enCicloAbierto == 1) {
      ...
      WebSerial.println("Contenedor abriendose");
    } else if (comandoAnterior == "abrir" && comando != "cerrar") {
